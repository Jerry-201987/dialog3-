...
// import { getLocale } from '../../utils/ows/store'
export default {
  props: {
    columns: {
      type: Array,
  ...
  // filters: {
  //   getTitle (item) {
  //     return getLocale() === 'en' ? item['title-en'] : item.title
  //   },
  // },
  watch: {
    columns(val) {
  ...
  mounted() {
    this.init()
    this.tableWidth = this.columns.reduce((pre, val) => pre + 1 * (val.width || 200), 0)
  },
  methods: {
    ...
    init() {
      this.$nextTick(() => {
        this.$emit('update:tableFilterShow', '')
        // const ucTable = this.$el
        // ucTable.addEventListener('scroll', this.handleScrol, true)
      })
    },
    toggleRowSelection(rows, flag = true) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.singleTable.toggleRowSelection(row, flag)
        })
      }
    },
    clearSelection() {
    ...
    //     map(expandedRows, this.$attrs['row-key']),
    //   )
    // },
    refresh() {
      this.$emit('update:tableFilterShow', '')
    ...
    renderColumn(h, colOptions) {
      ...
      const title = colOptions.title
      // label 共有3种方式获取: 1 title,title-en 2 lang(本地中英文) 3 label
      const label = title

      return colOptions.children && colOptions.children.length > 0 ? (
        <el-table-column label={label} align="center">
          {colOptions.children.map(v => this.renderColumnChild(h, v))}
        </el-table-column>
      ) : (
        <el-table-column
          prop={prop}
          ...
          // show-overflow-tooltip={!colOptions.tipHidden}
          label={label}
          min-width={colOptions.columnWidth}
          {...props}
          {...(colOptions.type === 'selection' ? {} : { scopedSlots })}
        />
      )
    },
    renderColumnChild(h, colOptions) {
      const { render } = colOptions
      ...
      const scopedSlots = {
        default(scope) {
          if (typeof render === 'function') {
            return render.apply(this, [h, scope])
          } else {
            return scope.row[colOptions.key]
          }
        }
  ...
  render(h) {
    return this.tableShow ? (
      <el-table
        ...
        header-row-class-name={this.headerRowClassName || 'mtable-header'}
        row-class-name={this.rowClassName || 'mtable-row'}
        ref="singleTable"
        {...{ on: this.$listeners }}
        on-sort-change={this.$attrs['sort-change'] || function() {}}
        // on-expand-change={this.expandChange}
        {...{ attrs: this.$attrs }}
      >
        {this.columns.map(v => this.renderColumn(h, v))}
      </el-table>
    ) : null
  }
}
